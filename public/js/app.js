// app.js — LOCKED IN client. router + views.
import { api, token, setToken, connectSocket, getSocket } from './api.js';
import { avatarSvg, stageForLevel } from './avatars.js';
import { sounds, confetti, shameFx, toast, flamePixels, isMuted, setMuted } from './fx.js';
...
let pendingStatus = null; // check-in selection before submit
...
$('#login-form').addEventListener('submit', async e => {
  ...
  btn.disabled = true;
  try {
    const data = await api('/' + loginMode, { method: 'POST', body: { username, pin, inviteCode } });
    setToken(data.token);
    me = await api('/me');
    sounds.yes();
    enterApp();
...
function renderTopbar() {
  const u = me.user;
  $('#chip-days').textContent = config.daysLeft;
  $('#chip-streak').textContent = u.streak;
  $('#chip-aura').textContent = u.aura;
  $('#chip-coins').textContent = u.coins;
...
function popChip(id) {
  const elx = $(id);
  elx.classList.remove('num-pop');
  void elx.offsetWidth;
  elx.classList.add('num-pop');
}

async function refreshMe() {
  const prev = me ? { ...me.user } : null;
  me = await api('/me');
  renderTopbar();
  if (prev) {
    if (me.user.aura !== prev.aura) popChip('#chip-aura');
    if (me.user.coins !== prev.coins) popChip('#chip-coins');
    if (me.user.streak !== prev.streak) popChip('#chip-streak');
  }
}
...
function setTab(name) {
  ...
  const view = $('#view');
  view.innerHTML = '';
  ({ home: renderHome, chat: renderChat, board: renderBoard, stats: renderStats, shop: renderShop })[name](view);
}
...
function setupSocket() {
  connectSocket({
    'chat:new': msg => {
      ...
      if (msg.type === 'system') {
        toast(msg.text, { kind: msg.text.includes('aura') && msg.text.includes('−') ? 'red' : '' });
      }
    },
    ...
    'squad:update': async () => {
      await refreshMe();
      // don't yank the view out from under someone mid-typing
    ...
    'fx': f => {
      if (f.userId === me.user.id) {
        if (f.kind === 'levelup') { confetti(140); sounds.levelup(); }
...
async function renderHome(view) {
  ...
  const days = el('div', 'hero-days', String(sq.daysLeft));
  ...
  // tribunal: excuses awaiting judgment
  ...
  tg.appendChild(el('div', 'card-title', `the squad today — ${new Date().toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric' }).toLowerCase()}`));
  ...
  for (const g of today.grid) {
    ...
    else sub.textContent = 'no word yet';
...
function buildCheckinCard() {
  ...
  if (c && !c.auto) {
    ...
    const st = el('div', `ci-done-status s-${c.status}`,
      { yes: 'you went. noted.', rest: 'rest day. logged.', no: 'a no. the tribunal has it.', skip: 'skipped. witnesses: everyone.' }[c.status]);
    done.appendChild(st);
  ...
  // not answered yet — the four buttons
...
function renderCheckinForm(form) {
  ...
  if (status === 'yes') {
    ...
    form.appendChild(submitBtn('log the workout', () => submitCheckin('yes', { description: desc.value })));
  } else if (status === 'rest') {
    form.appendChild(el('div', 'ci-hint', 'rest days are rationed: 2 per week. a third becomes a punished skip. +15 xp if legit.'));
    form.appendChild(submitBtn('take the rest day', () => submitCheckin('rest', {})));
  } else if (status === 'no') {
    ...
    form.appendChild(submitBtn('file the excuse', () => {
    ...
    form.appendChild(el('div', 'ci-hint', '−25 xp · −150 aura (double in winter arc) · streak dies · wall of shame. last chance to pick literally anything else.'));
    const b = submitBtn('skip anyway', () => submitCheckin('skip', {}));
    b.querySelector('button').classList.add('btn-danger');
    form.appendChild(b);
  }
}
...
async function submitCheckin(status, extra) {
  try {
    ...
    pendingStatus = null;
    if (r.levelUp) { confetti(160); sounds.levelup(); }
    else if (status === 'yes') sounds.yes();
...
function appendMessage(scroll, m, checkDaySep) {
  ...
  if (m.type === 'card') {
    ...
    const b = el('b', null, (m.meta.username || (m.user && m.user.username) || '?') + ' went gym');
    head.appendChild(b);
  ...
  const bubble = el('div', 'msg-bubble', m.text);
...
function addReactionUi(row, m) {
  ...
  row.addEventListener('click', e => {
    if (e.target.closest('.rx-chip') || e.target.closest('.rx-add')) return;
  });
}
...
// ============================================================
// BOARD (ranks / hall / bets)
// ============================================================
let boardSub = 'ranks';
async function renderBoard(view) {
  const subtabs = el('div', 'subtabs');
  for (const [key, label] of [['ranks', 'ranks'], ['hall', 'fame & shame'], ['bets', 'bets & callouts']]) {
...
async function renderRanks(body) {
  ...
  rows.forEach((r, i) => {
    ...
    row.appendChild(name);
    const s1 = el('div', 'lb-stat');
    s1.appendChild(el('b', null, String(r.score)));
    s1.appendChild(document.createTextNode(lbPeriod === 'week' ? 'xp this wk' : 'total xp'));
    row.appendChild(s1);
    const s2 = el('div', 'lb-stat');
    s2.appendChild(el('b', null, String(r.aura)));
    s2.appendChild(document.createTextNode('aura'));
    row.appendChild(s2);
    const s3 = el('div', 'lb-stat');
    s3.appendChild(el('b', null, String(r.streak)));
    s3.appendChild(document.createTextNode('streak'));
    row.appendChild(s3);
...
async function renderBets(body) {
  const [wagers, callouts, sq] = await Promise.all([api('/wagers'), api('/callouts'), squad ? Promise.resolve(squad) : api('/squad')]);
  squad = sq;

  // create wager
  const create = el('div', 'card wform');
  create.appendChild(el('div', 'card-title', 'open a wager'));
  ...
  cc.appendChild(el('div', 'card-title', 'formal callouts — one per week. choose wisely.'));
...
function wagerCard(w) {
  ...
  for (const m of w.members) {
    const chip = el('span', 'wager-member' + (m.is_loser ? ' loser' : '') + (m.paid ? ' paid' : ''),
...
async function renderStats(view) {
  ...
  const isMe = statsUserId === me.user.id;
  ...
  // weekly receipt — screenshot-native
  ...
  const rl1 = el('div', 'receipt-line'); rl1.appendChild(el('span', null, 'xp this week')); rl1.appendChild(el('b', null, String(s.week.xp)));
  const rl2 = el('div', 'receipt-line'); rl2.appendChild(el('span', null, 'rest days used')); rl2.appendChild(el('b', null, s.week.restUsed + ' / 2'));
  const rl3 = el('div', 'receipt-line'); rl3.appendChild(el('span', null, 'aura balance')); rl3.appendChild(el('b', null, String(s.user.aura)));
  receipt.appendChild(rl1); receipt.appendChild(rl2); receipt.appendChild(rl3);
  ...
  hm.appendChild(buildHeatmap(s.calendar, s.user.joined));
  ...
  if (s.recent.length) {
    ...
    for (const ev of s.recent) {
      ...
      const note = el('span', 'ledger-note', `${ev.kind.replace(/_/g, ' ')}${ev.note ? ' · ' + ev.note : ''}`);
...
function buildHeatmap(calendar, joined) {
  const wrap = el('div', 'heatmap');
  const today = new Date();
  // align to Monday, 17 weeks back
...
async function renderShop(view) {
  ...
  bal.appendChild(el('b', null, String(shop.coins)));
...
async function openProfile() {
  ...
  top.appendChild(el('div', 'profile-rank' + (u.level.name === 'HIM' ? ' lv-him' : ''), `lv.${u.level.lv} ${u.level.name}`));
  modal.appendChild(top);

  // xp progress
  const bar = el('div', 'xpbar');
  const fill = el('div', 'xpbar-fill');
  fill.style.width = Math.round(u.level.progress * 100) + '%';
  bar.appendChild(fill);
  ...
  for (const lvl of config.levels) {
    ...
    const sv = el('div');
    sv.innerHTML = avatarSvg(u.id, lvl.lv, { grayscale: u.level.lv < lvl.lv });
  ...
  soundBtn.style.cssText = 'width:100%;margin-top:14px';
  ...
  root.addEventListener('click', e => { if (e.target === root) root.classList.add('hidden'); }, { once: true });
}
